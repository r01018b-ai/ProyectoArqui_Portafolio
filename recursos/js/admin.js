...
const ADMIN = {
  ...
  renderDashboard() {
    ...
    const msgs  = DB.getMensajes().length;
    ...
    if (!rc) return;
    rc.innerHTML = DB.getCursos().map(function(c) {
  ...
  actualizarAvatar() {
    ...
    if (p.fotoUrl) {
      var img = '<img src="' + p.fotoUrl + '" alt="Foto" style="width:100%;height:100%;object-fit:cover;border-radius:inherit">';
  ...
  cambiarFoto(input) {
    ...
    reader.onload = function(e) {
      var p = DB.getPerfil(); p.fotoUrl = e.target.result;
      DB.savePerfil(p); self.actualizarAvatar();
  ...
  subirCV(input) {
    ...
    var reader = new FileReader();
    reader.onload = function(e) {
      var p = DB.getPerfil(); p.cvUrl = e.target.result; p.cvNombre = file.name;
      DB.savePerfil(p);
  ...
  cargarDatosPerfil() {
    ...
    var st = document.getElementById('cvStatus');
    if (st && p.cvNombre) st.innerHTML = '✅ <strong>' + p.cvNombre + '</strong>';
  },
  ...
  guardarPerfil(e) {
    ...
    p.nombre = n; p.apellido = a; p.nombreCompleto = n + ' ' + a;
  ...
  cargarChipsSemanas() {
    ...
    var curso   = cursoId ? DB.getCurso(cursoId) : null;
    var self    = this;
    var html    = '';
    for (var i = 0; i < 16; i++) {
      var n   = i + 1;
  ...
  registrarArchivo() {
    ...
    function guardarConDataUrl(dataUrl, extras) {
      var archivoObj = {
        id:          generateId(),
        archivo:     file.name,
        nombre:      nombre,
      ...
      try {
        var ok = DB.addArchivo(cursoId, semana, archivoObj);
        if (ok) {
        ...
        var ok2 = DB.addArchivo(cursoId, semana, archivoObj);
        if (ok2) {
          window._JZ_FILE_CACHE[archivoObj.id] = archivoObj;
    ...
    if (file.size <= LIMITE_FULL) {
      ...
      var reader = new FileReader();
      reader.onload = function(e) { guardarConDataUrl(e.target.result, null); };
      ...
      rText.onload = function(e) {
        ...
        r2.onload  = function(e2) { guardarConDataUrl(e2.target.result, { truncado: isTruncated }); };
  ...
  _descargarArchivo(id) {
    var archivo = window._JZ_FILE_CACHE[id];
    if (!archivo) { TOAST.show('Archivo no encontrado', 'error'); return; }
    var a      = document.createElement('a');
    a.href     = archivo.dataUrl || archivo.objectUrl || ('../' + archivo.ruta);
  ...
  guardarEdicionArchivo() {
    ...
    var ok = DB.updateArchivo(cursoId, semana, id, { nombre:nombre, descripcion:desc });
    if (ok) {
  ...
  ────────────────────────────────────────── */
  pedirEliminarArchivo(cursoId, semanaNum, archivoId) {
    ...
    pedirConfirm('🗑', '¿Eliminar archivo?',
      ...
      function() { ADMIN.eliminarArchivo(cursoId, semanaNum, archivoId); }
    );
  },

  eliminarArchivo(cursoId, semanaNum, archivoId) {
    DB.deleteArchivo(cursoId, semanaNum, archivoId);
  ...
  ────────────────────────────────────────── */
  agregarCurso() {
    var id     = (document.getElementById('cIdCurso')     || {}).value || '';
    ...
    id = id.trim(); nombre = nombre.trim(); desc = desc.trim(); icono = icono.trim();

    if (!id)     { TOAST.show('El ID es obligatorio', 'error'); return; }
    if (!nombre) { TOAST.show('El nombre es obligatorio', 'error'); return; }
    if (!/^[a-z0-9-]+$/.test(id)) { TOAST.show('ID: solo minúsculas, números y guiones', 'error'); return; }

    var cursos = DB.getCursos();
    if (cursos.find(function(c){ return c.id === id; })) { TOAST.show('Ya existe un curso con ese ID', 'error'); return; }

    var semanas = [];
    for (var i = 0; i < 16; i++) {
      semanas.push({ numero: i+1, titulo: 'Semana ' + (i+1), archivos: [] });
    }
    cursos.push({ id:id, nombre:nombre, descripcion:desc, icono:icono, color:color, semanas:semanas });
    DB.saveCursos(cursos);

    ['cIdCurso','cNombreCurso','cDescCurso','cIconoCurso'].forEach(function(elId) {
    ...
    TOAST.show('📚 Curso "' + nombre + '" creado', 'success');
  ...
  guardarEdicionCurso() {
    ...
    if (!nombre) { TOAST.show('El nombre es obligatorio', 'error'); return; }
    if (DB.updateCurso(id, { nombre:nombre, descripcion:desc, icono:icono, color:color })) {
      cerrarCursoModal();
  ...
  pedirEliminarCurso(id) {
    ...
    pedirConfirm('⚠️', '¿Eliminar curso?',
      ...
      function() { ADMIN.eliminarCurso(id); }
    );
  },
  ...
  agregarSemana(cursoId) {
    if (!cursoId) return;
    if (DB.addSemana(cursoId)) {
  ...
  pedirEliminarSemana(cursoId, semanaNum) {
    var curso = DB.getCurso(cursoId);
    ...
    pedirConfirm('🗑', '¿Eliminar semana?',
      ...
      function() { ADMIN.eliminarSemana(cursoId, semanaNum); }
    );
  },

  eliminarSemana(cursoId, semanaNum) {
    if (DB.deleteSemana(cursoId, semanaNum)) {
      this.renderSemanasAdmin();
      this.renderDashboard();
      this.renderListaArchivos();
  ...
  guardarEdicionSemana() {
    ...
    if (DB.updateSemana(cursoId, num, { titulo:titulo })) {
  ...
  renderSkills() {
    ...
    if (!el) return;
    var skills = DB.getPerfil().skills || [];
    if (!skills.length) {
  ...
  editarSkill(idx) {
    var p = DB.getPerfil(); var skill = p.skills[idx]; if (!skill) return;
    ...
    p.skills[idx].nivel = nv; DB.savePerfil(p); this.renderSkills();
    TOAST.show('Nivel actualizado ✅', 'success');
  },

  eliminarSkill(idx) {
    var p = DB.getPerfil(); var name = p.skills[idx] ? p.skills[idx].nombre : '';
    pedirConfirm('🗑', '¿Eliminar habilidad?', 'Se eliminará "' + name + '".',
      function() { p.skills.splice(idx,1); DB.savePerfil(p); ADMIN.renderSkills(); TOAST.show('Habilidad eliminada', 'info'); }
    );
  },
  ...
  eliminarMensaje(id) {
    DB.saveMensajes(DB.getMensajes().filter(function(m){ return m.id !== id; }));
  ...
  limpiarMensajes() {
    pedirConfirm('⚠️', '¿Limpiar todos los mensajes?',
      ...
      function() { DB.saveMensajes([]); ADMIN.renderMensajes(); ADMIN.actualizarDotMensajes(); TOAST.show('Bandeja limpiada 📭', 'info'); }
    );
  },

  actualizarDotMensajes() {
    var dot = document.getElementById('msgDot');
    if (dot) dot.style.display = DB.getMensajes().length ? 'inline-block' : 'none';
  },
